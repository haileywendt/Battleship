...
namespace Battleship
{
    internal class Battleship
    {
        ...
        private int minShip;                // min ship size that hasn't been found
        ...
        public void makeBtn(Form form, bool isComputer, int turn)
        {
            ...
            for (int i = 0; i < columns; i++)
            {
                for (int j = 0; j < rows; j++)
                {
                    ...
                    // sets the location of buttons depends on the board size
                    else
                    {
        ...
        private void fire(object sender, EventArgs e, Button[,] btns)
        {
            ...
            else
            {
                ...
                if (vsComputer)
                {
                    ...
                    // computer fire player's board depending on computer's difficulty
                    firePtr(player1);
        ...
        private void fireCompMedium(Button[,] playerBtns)
        {
            ...
            // here when the computer has found a button for ship
            // the computer will randomly shoot at surrouing 4
            else if (count == 1)
            {
                int countLoop = 0;
                // randomly selets which surrounding spot to shoot at until it's valid
                do
                {
                    int r1;
                    int r2 = rnd.Next() % 2;

                    // if there is not enough spots for ships neither horizontally or vertically
                    // it will randomly pick a surrounding spot to shoot at
                    if (horizontalCheck == false && verticalCheck == false)
                    {
                        r1 = rnd.Next() % 2;
                    }
                    // if there is not enough spots horizontally
                    // it will only shoot verically from the ship that was found in count = 1
                ...
                // when none of the surrounding spots were available
                if (countLoop >= 20)
                {
                    // it will randomly choose another spot to shoot
                    fireRandomPtr(playerBtns);
                }
                // n1 and n2 is set to i and j respectively
                // so that it knows where to shoot next if the shot was successful
                else
                {
                    n1 = i;
                    n2 = j;
            ...
            // when 2 spots for ships are found adjacently
            else if (count == 2)
            {
                // increaments in a direction where second spot was found
                i += n1;
                j += n2;

                // if it's not valid, it checks on the opposite direction of where the first spot was found
                if (!isValidComp(playerBtns, x + i, y + j))
                {
                    // if it's not valid either, 
                    if (isValidComp(playerBtns, x - n1, y - n2))
                    {
                        // updates i and j to the opposite direction
                        i = -n1;
                        j = -n2;
                        n1 = i;
                        n2 = j;
                    ...
                    // if it's not valid either, it randomly shoots
                    else
                    {
                        count = 0;
                        shotTemp = 0;
            ...
            // here if count = 3
            // here when it's checking the other direction after count = 2
            else
            {
                // keeps increamenting i and j to the same direction
                i += n1;
                j += n2;
                // if it's no longer valid, it will randomly shoot
                if (!isValidComp(playerBtns, x + i, y + j))
                {
                    count = 0;
                    shotTemp = 0;
            ...
            // if there is a ship, turn the button to red indicating there is a ship
            if (isShip(playerBtns[x + i, y + j]))
            {
                ...
                // increments the count if less than 2
                // because it increments the count from 2 to 3 when it wasn't ship
                if (count < 2)
                {
                    count++;
                }
                // if temporary shot count gets greater than the maximum ship size
                // if will reset the count because all spots for a ship has been found
                if (shotTemp >= maxShip)
                {
                    count = 0;
                    shotTemp = 0;
            ...
            else
            {
                ...
                // if missed when count is 2, it will start checking opposite direction
                if (count == 2)
                {
                    count++;
                    n1 = -n1;
                ...
                // if missed when count is 3, that means it has already found all spots for a ship
                // so it resets the count to 0 and start randomly shooting again
                else if (count == 3)
                {
                    count = 0;
                    shotTemp = 0;
        ...
        {
            // spaces left from the button where the computer is trying to shoot
            int tempHorizontal;
            int tempVertical;
            // how many times the loop runs
            int innerLoop;

            // keeps checking spaces left horizontally and vertically from the random spot(x,y)
            // to see if there can be a ship
            do
            {
                innerLoop = 0;
                tempHorizontal = 0;
                ...
                // keeps checking the space downward from (x,y)
                for (int i = 0; i < maxShip; i++)
                {
                    // keeps incrementing the space until it's no longer valid
                ...
                // keeps checking the space upward from (x,y)
                for (int i = 0; i < maxShip; i++)
                {
                    if (isValidComp(playerBtns, x - i, y))
                    {
                ...
                // keeps checking the space to the right from (x,y)
                for (int i = 0; i < maxShip; i++)
                {
                ...
                // keeps checking the space to the left from (x,y)
                for (int i = 0; i < maxShip; i++)
                {
                    if (isValidComp(playerBtns, x, y - i))
                    {
            ...
            // if there is a less spaces than minimum size of the ship horizontally or vertically
            // it randomly fires
            if (tempHorizontal < minShip && tempVertical < minShip)
            {
                fireRandomPtr(playerBtns);
            ...
            // if there is a space horizontally
            // it will turn horizontal check to true so that
            // it will shoot horizontally next if there is a ship
            if (tempHorizontal >= minShip)
            {
                horizontalCheck = true;
            }

            // if there is a space vertically
            // it will turn vertical check to true so that
            // it will shoot vertically next if there is a ship
            if (tempVertical >= minShip)
            {
                verticalCheck = true;
            }

            // resets i and j to 0 so that it will shoot at (x,y)
            // because it will shoot at (x+i, y+j)
            i = 0;
            j = 0;
        }
        ...
        private bool overlap(int x, int y, String[,] str)
        {
            ...
            if (horizontal)
            {
                // traverse horizontally from where the user clicked by size of ship
            ...
            else
            {
                for (int i = 0; i < size; i++)
                {
                    // traverse vertically from where the user clicked by size of ship
                    // and if any places are overlapped, return true, otherwise return false
        ...
        private bool outOfGrid(int x, int y)
        {
            ...
            // the end of the ship goes outside of the board
        ...
        private void setShip(bool horizontal, int x, int y, Button[,] btns, String[,] str, bool isComputer)
        {
            ...
            if (horizontal)
            {
                // turn all the button where the ship will be placed to red
                // and set the string at each index to SHIP so that the computer knows
                // where the ships are after resetting the button color
                for (int i = 0; i < size; i++)
                {
                    str[x + i, y] = SHIP;
                    if (!isComputer)
        ...
        public void setShipComp()
        {
            ...
            // randomly select a position to set a ship
            ...
            {
                size = i;
                // randomly select if the ship will be placed horizontally or vertically
                if (rnd.Next() % 2 == 0)
                {
                    horizontal = true;
                }
                ...
                // if the randomly selected button is not valid
                // randomly select another position to set ship until it's valid
        ...
        // when the rotation button is clicked, determine if it's horizontal or not
        ...
        {
            readyBtn.Show();
            // sets n to number of ships to enable ready btn after placing all the ships
